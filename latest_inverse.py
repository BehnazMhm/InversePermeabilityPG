...
def plot_predicted_vs_actual(model, test_loader, scaler_x0, param_cols, outdir, model_name="best"):
    """
    Create predicted vs actual plots for each physical parameter.
    ...
    if not HAS_MATPLOTLIB:
        print("Skipping plotting: matplotlib is not installed. Install with: pip install matplotlib")
        return
    ...
    for i, param_name in enumerate(param_cols):
        ...
        # Labels and title
        ax.set_xlabel(f'Actual {param_name}', fontsize=10)
        ax.set_ylabel(f'Predicted {param_name}', fontsize=10)
        ax.set_title(f'{param_name}\n(R² = {r2_score(actual, pred):.4f})', fontsize=11)
        ax.legend()
    ...
    plt.tight_layout()
    plot_path = os.path.join(outdir, f'predicted_vs_actual_{model_name}.png')
    plt.savefig(plot_path, dpi=150, bbox_inches='tight')
    print(f"Saved predicted vs actual plots to: {plot_path}")
    plt.close()
...
def test_current_and_best(trainer, model, test_loader, best_ckpt_path, forward_model, scaler_x0, param_cols, outdir):
    ...
    # Create predicted vs actual plots for best model
    print("\n== Generating predicted vs actual plots ==")
    plot_predicted_vs_actual(best_model, test_loader, scaler_x0, param_cols, outdir, model_name="best")
    
    # Create individual sample comparison plots
    print("\n== Generating individual sample plots ==")
    plot_individual_samples(best_model, test_loader, scaler_x0, param_cols, outdir, model_name="best", max_samples=50)
