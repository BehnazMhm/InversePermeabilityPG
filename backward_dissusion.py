...
#         with torch.no_grad():
#             x0_gen = self.sample(cond, num_steps=50)
...
#     @torch.no_grad()
#     def sample(self, cond: torch.Tensor, num_steps: int = 50) -> torch.Tensor:
#         """
...
#         # Time grid from 1.0 down to a small epsilon (same as training range)
#         t_eps = 1e-4
#         times = torch.linspace(1.0, t_eps, steps=num_steps, device=device)
#         dt = times[0] - times[1]  # positive step size

#         for i, cur_t in enumerate(times):
#             # Current time scalar -> [B]
#             t_batch = torch.full((B,), cur_t.item(), device=device)
            
#             # Predict clean x0 at time cur_t
#             x0_hat = self.model(x, cond, t_batch)  # [B, param_dim]
            
#             # Current noise level
#             sigma_t = self.sigma_min + (self.sigma_max - self.sigma_min) * cur_t
            
#             if i == len(times) - 1:
#                 # Last step: just use the prediction
#                 x = x0_hat
#                 break
            
#             # Next time step
#             next_t = times[i + 1]
#             sigma_next = self.sigma_min + (self.sigma_max - self.sigma_min) * next_t
            
#             # DDIM update: x_{t-dt} = x0_hat + (sigma_{t-dt} / sigma_t) * (x_t - x0_hat)
#             # This is equivalent to: x = x0_hat + (sigma_next / sigma_t) * (x - x0_hat)
#             if sigma_t > 1e-6:
#                 x = x0_hat + (sigma_next / sigma_t) * (x - x0_hat)
#             else:
#                 x = x0_hat
...
class InverseDiffusionLightning(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        # Generate samples and compute reconstruction metrics
        # Use more steps for better quality (can be increased further)
        with torch.no_grad():
            x0_gen_50 = self.sample(cond, num_steps=50)
            x0_gen_100 = self.sample(cond, num_steps=100)  # Try with more steps
            
            # Metrics in standardized space
            mse_50 = (x0_gen_50 - x0).pow(2).mean()
            mae_50 = (x0_gen_50 - x0).abs().mean()
            mse_100 = (x0_gen_100 - x0).pow(2).mean()
            mae_100 = (x0_gen_100 - x0).abs().mean()
            
            self.log("test/mse_gen", mse_50, prog_bar=True, on_epoch=True)
            self.log("test/mae_gen", mae_50, prog_bar=True, on_epoch=True)
            self.log("test/mse_gen_100", mse_100, prog_bar=False, on_epoch=True)
            self.log("test/mae_gen_100", mae_100, prog_bar=False, on_epoch=True)

    @torch.no_grad()
    def sample(self, cond: torch.Tensor, num_steps: int = 50) -> torch.Tensor:
        """
        DDIM-style deterministic sampler using Claim 2 from "Step-by-Step Diffusion" tutorial.
        
        Uses the formula: E[x_{t-dt} | x_t] = (dt/t) * E[x_0 | x_t] + (1 - dt/t) * x_t
        ...
        for i, cur_t in enumerate(times):
            ...
            # Apply Claim 2 from "Step-by-Step Diffusion" tutorial:
            # E[x_{t-dt} | x_t] = (dt/t) * E[x_0 | x_t] + (1 - dt/t) * x_t
            # This gives us the expected value of x at the next time step
            t_scalar = cur_t.item()
            if t_scalar > 1e-6:  # Avoid division by zero
                # dt/t is the weight for x0_hat, (1 - dt/t) is the weight for x_t
                x = (dt.item() / t_scalar) * x0_hat + (1.0 - dt.item() / t_scalar) * x
            else:
                # If t is very small, just use the prediction
                x = x0_hat
